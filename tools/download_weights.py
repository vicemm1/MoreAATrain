...
# 配置自动下载训练poseguider时需要的模型
def prepare_open_pose():
    print(f'Preparing  openpose controlnet weights...')
...
# 配置自动下载训练poseguider时需要的模型
def prepare_control_v11p():
    local_dir = "./pretrained_weights/control_v11p_sd15_openpose/"
    os.makedirs(local_dir, exist_ok=True)
    for hub_file in ["config.json", "diffusion_pytorch_model.bin"]:
        path = Path(hub_file)
        saved_path = local_dir / path
        if os.path.exists(saved_path):
            continue
