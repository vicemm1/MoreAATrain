...
# 当出现找不到src module错误时，运行命令 export PYTHONPATH=$(pwd) 将项目引入环境变量
...
if __name__ == "__main__":
    ...
    # split into chunks,
    # 降低batchsize 4090 24GB(出现爆显存的情况，数据集单个视频过大)
    # 实际测试 一个视频进行骨架提取要 1-2G左右，由于线程池的开启，可能会占据更大空间，而且部分操作可能会出现占用显存暴涨，单卡建议batchsize 1 
